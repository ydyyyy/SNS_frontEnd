    ...
    <el-container class="me-allct-container">
      <el-main>
        <el-tabs v-model="activeName">
          <el-tab-pane label="公司" name="category">
          ...
          <el-tab-pane label="技术栈" name="tag">
            <ul class="me-allct-items">
              <li
                v-for="t in tags"
                @click="view(t.id)"
...
export default {
  ...
  methods: {
    view(id) {
      this.$router.push({
        path: `/recommend/type/${this.currentActiveName}/${id}`,
      });
    },
    getCategorys() {
      let that = this;
      getAllIndustryDetail()
        .then((data) => {
          ...
          // 遍历 that.tags 数组并更新每个对象的 avatar 属性
          that.categorys.forEach((tag) => {
            tag.avatar = `/static${tag.avatar}`;
          });
        })
        .catch((error) => {
          if (error !== "error") {
    ...
    getTags() {
      ...
      getAllStackDetail()
        .then((data) => {
          console.log(data, "我是getAllStackDetail获取的数据");
          that.tags = data.data;
          // 遍历 that.tags 数组并更新每个对象的 avatar 属性
          that.tags.forEach((tag) => {
            tag.avatar = `/static${tag.avatar}`;
          });
          console.log(that.tags, "我是更新后的tags数组");
        })
        .catch((error) => {
          if (error !== "error") {
            that.$message({
